...
class _InputRenderable:
    ...
    def __init__(self, input: AlwaysInput, cursor_visible: bool) -> None:
        self.input = input
    ...
    def __rich_console__(
        ...
        if self.cursor_visible:  # edited and input.has_focus:
...
class AlwaysInput(Widget, can_focus=True):
    """A text input widget."""
    ...
    AlwaysInput>.input--cursor {
        background: $surface;
        color: $text;
        text-style: reverse;
    }
    AlwaysInput>.input--placeholder, Input>.input--suggestion {
        color: $text-disabled;
    }
    AlwaysInput.-invalid {
        border: tall $error 60%;
    }
    AlwaysInput.-invalid:focus {
        border: tall $error;
    }
    """
    ...
    def _on_blur(self, event: Blur) -> None:
        # edited self._pause_blink_cycle()
